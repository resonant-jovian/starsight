...
impl Figure {
    ...
    pub fn render_png(&self) -> Result<Vec<u8>> {
        ...
        let extent = self.merged_extent().ok_or_else(|| {
            starsight_layer_1::errors::StarsightError::Data("No data to render".into())
        })?;
        ...
        let x_axis = Axis::auto_from_data(&x_vals, 5).ok_or_else(|| {
            starsight_layer_1::errors::StarsightError::Scale("Cannot build X axis".into())
        })?;
        let y_axis = Axis::auto_from_data(&y_vals, 5).ok_or_else(|| {
            starsight_layer_1::errors::StarsightError::Scale("Cannot build Y axis".into())
        })?;
        ...
        crate::renders::render_background(&plot_area, &mut backend)?;
        crate::renders::render_axes(&coord, &mut backend)?;
    ...
    ///
    /// # Errors
    /// - Any error from [`Figure::render_png`].
    /// - [`StarsightError::Io`](starsight_layer_1::errors::StarsightError::Io)
    ///   if writing the file fails.
    pub fn save(&self, path: impl AsRef<std::path::Path>) -> Result<()> {
        let bytes = self.render_png()?;
        std::fs::write(path, bytes).map_err(starsight_layer_1::errors::StarsightError::Io)
    }
}
